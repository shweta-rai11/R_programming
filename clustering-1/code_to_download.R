...
class(eset)
# Save expression set
save( eset, file="GSE13837_ESet.RData" )

# Save the expression matrix
write.csv(exprs(eset), file="GSE13837_expressions.csv")

# Get sample data
write.csv(pData(eset), file="GSE13837_phenodata.csv")
...
head(annotation)[1:3,]
# Save annotation
write.csv(annotation, file="GSE13837_annotation.csv")
