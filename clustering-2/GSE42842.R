...
setwd("~/Desktop/GSE42842/GSE42842_RAW")

# Load GSE42842 data set using GEOquery, here I loaded saved 
# data object from my system
load("/Users/swetarai/desktop/GSE42842/GSE42842_RAW/GSE42842_ESet.RData")
show(eset)
...
sample_data <- data.frame( treatment = sample_data$characteristics_ch1.3,
                           Gender = sample_data$characteristics_ch1,
                           efficacy = sample_data$characteristics_ch1.4)
head(sample_data)
tail(sample_data)

# Modify the variable definitions to make them more palatable
sample_data$treatment[sample_data$treatment == "treatment: methotrexate +  adalimumab"] <- "methotrexate_adalimumab"
...
table(sample_data$treatment, sample_data$Gender)
# Female Male
# Control     10    5
# RA          14    4
...
arrayQualityMetrics(eset,
                    outdir = "GSE42842_arrayQualityReport",
                    force = TRUE,
...
pdf("GSE42842_heatmap_top500.pdf", width = 7, height = 6)
pheatmap(top500, 
